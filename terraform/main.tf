...
resource "aws_lb_listener" "listener" {
  load_balancer_arn = "${aws_alb.awsplay_lb.arn}" # Referencing our load balancer
  port              = "443"
  protocol          = "HTTPS"
  ...
  default_action {
    type             = "forward"
    target_group_arn = "${aws_lb_target_group.fargate.arn}" # Referencing our tagrte group
  }
}

resource "aws_ecs_service" "awsplay_service" {
  name            = "awsplay-service"
  cluster         = "${aws_ecs_cluster.awsplay.id}"             # Referencing our created Cluster
  task_definition = "${aws_ecs_task_definition.awsplay.arn}"    # Referencing the task our service will spin up
  launch_type     = "FARGATE"
  desired_count   = 1

  load_balancer {
    target_group_arn = "${aws_lb_target_group.fargate.arn}" # Referencing our target group
  ...
  network_configuration {
    ...
    security_groups  = ["${aws_security_group.awsplay_fargate_security_group.id}"] # Setting the security group
  }
}
